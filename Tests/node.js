                    ...
                    .then(SpecificUser => {
                        if (SpecificUser != null) {
                            if ((HandledRequest.body.Message.data).length == 512) {
                                if (crypto.privateDecrypt({ key: Key.dataValues.PrivateKey, passphrase: Key.dataValues.Passphrase }, Buffer.from(HandledRequest.body.Message)).equals(crypto.privateDecrypt({ key: Key.dataValues.PrivateKey, passphrase: Key.dataValues.Passphrase }, SpecificUser.Message))) {
                                    if (hash.copy().update(HandledRequest.body.MasterPw + SpecificUser.dataValues.Salt).digest('hex') == SpecificUser.MasterPw) {
                                        done()
                                    } else
                                        done(hash.copy().update(HandledRequest.body.MasterPw + SpecificUser.dataValues.Salt).digest('hex') == SpecificUser.MasterPw);
                                } else
                                    done(crypto.privateDecrypt({ key: Key.dataValues.PrivateKey, passphrase: Key.dataValues.Passphrase }, Buffer.from(HandledRequest.body.Message)).equals(crypto.privateDecrypt({ key: Key.dataValues.PrivateKey, passphrase: Key.dataValues.Passphrase }, SpecificUser.Message)));
                            } else
                                done(Error((HandledRequest.body.Message.data).length));
                        } else
